export const PROFESSIONAL_CURATION_VERSION = 'professional-curation-v5-backup-snapshot-2026-05-31';

export const SCORE_OVERRIDES = {
  'tokyo-nakamise-dori': 8,
  'tokyo-hoppy-street': 7,
  'tokyo-kappabashi': 8,
  'tokyo-shibuya-sky': 10,
  'tokyo-mega-donki': 8,
  'tokyo-omoide-yokocho': 9,
  'tokyo-shinjuku-gyoen': 8,
  'tokyo-super-potato': 8,
  'tokyo-bic-camera': 7,
  'tokyo-yanaka': 8,
  'kyoto-yasaka-shrine': 8,
  'kyoto-nishiki': 9,
  'kyoto-tower': 6,
  'kyoto-sake-district': 8,
  'osaka-glico': 8,
  'osaka-namba-parks': 7,
  'osaka-takimi-koji': 8,
  'osaka-gate-tower': 6,
  'osaka-bigman': 5,
  'osaka-tenmangu': 7,
  'osaka-goryo': 5,
  'osaka-usj': 10,
  'osaka-ajinoya': 9,
  'osaka-tendon-makino': 8,
  'osaka-551-horai': 9,
  'osaka-yp-ryu': 8,
  'osaka-rikuro': 8,
  'osaka-gorichan': 7,
  'takayama-sanmachi-suji': 10,
  'takayama-miyagawa-market': 9,
  'takayama-jinya': 9,
  'takayama-hida-no-sato': 9,
  'takayama-shirakawago-day-trip': 10,
  'takayama-hida-beef': 9
};

export const RAINY_FRIENDLY_IDS = [
  'tokyo-nakamise-dori',
  'tokyo-skytree',
  'tokyo-mega-donki',
  'tokyo-metropolitan-govt',
  'tokyo-akihabara',
  'tokyo-super-potato',
  'tokyo-bic-camera',
  'tokyo-teamlab',
  'tokyo-odaiba',
  'kyoto-ceremonia-te',
  'kyoto-botigues',
  'kyoto-nijo-castle',
  'kyoto-nishiki',
  'kyoto-tower',
  'kyoto-museo-nacional',
  'kyoto-manga-museum',
  'kyoto-estacion',
  'kyoto-sake-district',
  'osaka-kuromon',
  'osaka-shinsaibashi',
  'osaka-namba-parks',
  'osaka-umeda-sky',
  'osaka-station-city',
  'osaka-takimi-koji',
  'osaka-gate-tower',
  'osaka-bigman',
  'osaka-hep-five',
  'osaka-tsutenkaku',
  'osaka-abeno-harukas',
  'osaka-aquarium',
  'osaka-tempozan',
  'osaka-den-den',
  'osaka-ajinoya',
  'osaka-tendon-makino',
  'osaka-551-horai',
  'osaka-yp-ryu',
  'osaka-rikuro',
  'osaka-gorichan',
  'takayama-jinya',
  'takayama-matsuri-yatai-kaikan',
  'takayama-hida-beef',
  'takayama-sake-breweries',
  'takayama-ryokan-experience'
];

export const PLANNED_DAY_IDS = {
  1: [
    'osaka-namba-yasaka',
    'osaka-shinsaibashi',
    'osaka-hozenji',
    'osaka-glico',
    'osaka-dotonbori',
    'osaka-ajinoya',
    'osaka-rikuro'
  ],
  2: [
    'osaka-usj',
    'osaka-umeda-sky',
    'osaka-takimi-koji'
  ],
  3: [
    'osaka-castle',
    'osaka-nishinomaru',
    'osaka-kuromon',
    'osaka-den-den',
    'osaka-namba-parks'
  ],
  4: [
    'osaka-sumiyoshi',
    'osaka-shitennoji',
    'osaka-shinsekai',
    'osaka-tsutenkaku',
    'osaka-abeno-harukas'
  ],
  5: [
    'osaka-nakazakicho',
    'osaka-station-city',
    'kyoto-nijo-castle',
    'kyoto-nishiki',
    'kyoto-pontocho'
  ],
  6: [
    'kyoto-kiyomizu-dera',
    'kyoto-sannenzaka',
    'kyoto-hokanji',
    'kyoto-yasaka-koshin',
    'kyoto-gion',
    'kyoto-hanamikoji',
    'kyoto-yasaka-shrine'
  ],
  7: [
    'kyoto-fushimi-inari',
    'kyoto-ruta-torii',
    'kyoto-tofukuji',
    'kyoto-sake-district'
  ],
  8: [
    'kyoto-bambu',
    'kyoto-tenryuji',
    'kyoto-togetsukyo',
    'kyoto-kinkakuji',
    'kyoto-ryoanji'
  ],
  9: [
    'takayama-sanmachi-suji',
    'takayama-nakabashi-bridge',
    'takayama-sake-breweries',
    'takayama-hida-beef',
    'takayama-ryokan-experience'
  ],
  10: [
    'takayama-miyagawa-market',
    'takayama-jinya-mae-market',
    'takayama-jinya',
    'takayama-hida-no-sato',
    'takayama-shiroyama-park'
  ],
  11: [
    'takayama-shirakawago-day-trip',
    'takayama-matsuri-yatai-kaikan'
  ],
  12: [
    'takayama-hida-furukawa-day-trip',
    'tokyo-omoide-yokocho',
    'tokyo-golden-gai'
  ],
  13: [
    'tokyo-senso-ji',
    'tokyo-nakamise-dori',
    'tokyo-kappabashi',
    'tokyo-skytree',
    'tokyo-hoppy-street'
  ],
  14: [
    'tokyo-meiji-jingu',
    'tokyo-takeshita',
    'tokyo-shibuya-crossing',
    'tokyo-mega-donki',
    'tokyo-shibuya-sky'
  ],
  15: [
    'tokyo-ueno-park',
    'tokyo-yanaka',
    'tokyo-shinjuku-gyoen',
    'tokyo-metropolitan-govt'
  ],
  16: [
    'tokyo-teamlab',
    'tokyo-odaiba'
  ],
  17: [
    'tokyo-palacio-imperial',
    'tokyo-akihabara',
    'tokyo-super-potato',
    'tokyo-bic-camera'
  ]
};

export const DISCARDED_PLACE_IDS = [
  'kyoto-tower',
  'osaka-bigman',
  'osaka-goryo',
  'osaka-tennoji-park'
];
