export function filterPlaces(places, { search = '', category = '', priority = '', zone = '', timeOfDay = '', favoritesOnly = false, favorites = {}, rainyFriendly = false }) {
  return places.filter(p => {
    ...
    if (timeOfDay && p.bestTime && p.bestTime !== 'Cualquier momento') {
      const lowerTime = p.bestTime.toLowerCase();
      if (timeOfDay === 'mañana' && !lowerTime.includes('mañana') && !lowerTime.includes('madrugada')) return false;
      if (timeOfDay === 'tarde' && !lowerTime.includes('tarde') && !lowerTime.includes('atardecer')) return false;
      if (timeOfDay === 'noche' && !lowerTime.includes('noche') && !lowerTime.includes('nocturno')) return false;
    }
